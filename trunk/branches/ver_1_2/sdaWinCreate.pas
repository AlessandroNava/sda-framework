...
function SdaRegisterWindowClass(WndClass: TWndClassEx;
  sdaObjectClass: TSdaWindowObjectClass): Boolean;
...
begin
  ...
  begin
    sdaObj := SdaCreateAssociatedObject(hWnd);
...
var
  h: HWND;
begin
  WndClass.cbWndExtra := WndClass.cbWndExtra + SizeOf(Pointer); // Associated object
  WndClass.cbClsExtra := WndClass.cbClsExtra + SizeOf(Pointer); // Associated class
  WndClass.lpfnWndProc := @SdaWindowProc;
  Result := RegisterClassEx(WndClass) <> 0;
  if Result then
  begin
    h := CreateWindow(WndClass.lpszClassName, nil, WS_OVERLAPPED, 0, 0, 0, 0, 0, 0,
      WndClass.hInstance, nil);
    if h <> 0 then
    begin
      SetClassLong(h, WndClass.cbClsExtra - SizeOf(Pointer), NativeInt(sdaObjectClass));
      DestroyWindow(h);
    end;
  end;
end;
