...
procedure TSdaCanvas.AngleArc(const Center: TPoint; Radius: Cardinal;
  StartAngle, SweepAngle: Single);
begin
  Windows.AngleArc(FHandle, Center.X, Center.Y, Radius, StartAngle, SweepAngle);
end;

procedure TSdaCanvas.Arc(const Rect: TRect; const Start, Finish: TPoint);
begin
  Windows.Arc(FHandle, Rect.Left, Rect.Top, Rect.Right, Rect.Bottom,
    Start.X, Start.Y, Finish.X, Finish.Y);
end;

procedure TSdaCanvas.ArcTo(const Rect: TRect; const Start, Finish: TPoint);
begin
  Windows.ArcTo(FHandle, Rect.Left, Rect.Top, Rect.Right, Rect.Bottom,
    Start.X, Start.Y, Finish.X, Finish.Y);
end;
...
begin
  Windows.Chord(FHandle, Rect.Left, Rect.Top, Rect.Right, Rect.Bottom,
    Start.X, Start.Y, Finish.X, Finish.Y);
end;
...
begin
  Windows.Ellipse(FHandle, Rect.Left, Rect.Top, Rect.Right, Rect.Bottom);
end;
...
begin
  Windows.FillRect(FHandle, Rect, 0);
end;
...
begin
  Windows.FrameRect(FHandle, Rect, 0);
end;

function TSdaCanvas.GetArcDirection: TArcDirection;
begin
  if Windows.GetArcDirection(FHandle) = AD_CLOCKWISE then Result := arcClockWise
...
begin
  Windows.InvertRect(FHandle, Rect);
end;
...
begin
  Windows.LineDDA(Start.X, Start.Y, Finish.X, Finish.Y, @LineDDAProc,
    NativeInt(@Callback));
end;
...
begin
  Windows.Pie(FHandle, Rect.Left, Rect.Top, Rect.Right, Rect.Bottom,
    Start.X, Start.Y, Finish.X, Finish.Y);
end;
...
begin
  ...
  Windows.PolyBezier(FHandle, Points[0], Length(Points));
end;

procedure TSdaCanvas.PolyBezierTo(const Points: array of TPoint);
begin
  if Length(Points) <= 0 then Exit;
  Windows.PolyBezierTo(FHandle, Points[0], Length(Points));
end;

procedure TSdaCanvas.Polygon(const Points: array of TPoint);
begin
  if Length(Points) <= 0 then Exit;
  Windows.Polygon(FHandle, Points[0], Length(Points));
end;

procedure TSdaCanvas.Polyline(const Points: array of TPoint);
begin
  if Length(Points) <= 0 then Exit;
  Windows.Polyline(FHandle, Points[0], Length(Points));
end;

procedure TSdaCanvas.PolylineTo(const Points: array of TPoint);
begin
  if Length(Points) <= 0 then Exit;
  Windows.PolylineTo(FHandle, Points[0], Length(Points));
end;

procedure TSdaCanvas.Rectangle(const Rect: TRect);
begin
  Windows.Rectangle(FHandle, Rect.Left, Rect.Top, Rect.Right, Rect.Bottom);
end;
...
begin
  Windows.RoundRect(FHandle, Rect.Left, Rect.Top, Rect.Right, Rect.Bottom,
    Width, Height);
end;
...
begin
  if Value = arcClockWise then Windows.SetArcDirection(FHandle, AD_CLOCKWISE)
    else Windows.SetArcDirection(FHandle, AD_COUNTERCLOCKWISE);
end;
