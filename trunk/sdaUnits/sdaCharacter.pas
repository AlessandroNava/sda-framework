...
{$ENDIF}
{$IFDEF MSWINDOWS}
    if LCMapString(LOCALE_INVARIANT, LCMAP_LOWERCASE, PChar(S), Length(S), PChar(Result), Length(Result)) = 0 then
      RaiseLastOSError;
{$ENDIF}
  end;
end;
...
{$ENDIF}
{$IFDEF MSWINDOWS}
    if LCMapString(LOCALE_INVARIANT, LCMAP_UPPERCASE, PChar(S), Length(S), PChar(Result), Length(Result)) = 0 then
      RaiseLastOSError;
{$ENDIF}
  end;
end;
